...
mod tests {
    ...
    #[test]
    fn defaults_match_current_gameplay_layout() {
        let defaults = ControlsConfig::default();

        assert_eq!(defaults.solo, PlayerBindings::solo_default());
        assert_eq!(
            defaults.one_vs_one_p1,
            PlayerBindings::one_vs_one_p1_default()
        );
        assert_eq!(
            defaults.one_vs_one_p2,
            PlayerBindings::one_vs_one_p2_default()
        );
    }

    #[test]
    fn conflict_detection_finds_other_player_key() {
        let config = ControlsConfig::default();

        let conflict = config.one_vs_one_conflict_for(KeyCode::Left, ControlProfile::OneVsOneP1);

        assert_eq!(
            conflict,
            Some(ConflictInfo {
                conflicting_profile: ControlProfile::OneVsOneP2,
                conflicting_action: BindingAction::MoveLeft,
            })
        );
    }

    #[test]
    fn validation_rejects_shared_keys_between_players() {
        let mut config = ControlsConfig::default();
        config.one_vs_one_p2.move_left = KeyCode::Q;

        assert_eq!(
            config.validate(),
            Err(SettingsValidationError::OneVsOneConflict {
                first: BindingTarget {
                    profile: ControlProfile::OneVsOneP1,
                    action: BindingAction::MoveLeft,
                },
                second: BindingTarget {
                    profile: ControlProfile::OneVsOneP2,
                    action: BindingAction::MoveLeft,
    ...
    #[test]
    fn validation_rejects_escape() {
        let mut config = ControlsConfig::default();
        config.solo.shoot = KeyCode::Escape;

        assert_eq!(
            config.validate(),
            Err(SettingsValidationError::ForbiddenKey(KeyCode::Escape))
        );
    }

    #[test]
    fn set_binding_updates_requested_target() {
        let mut config = ControlsConfig::default();
        let target = BindingTarget {
            profile: ControlProfile::OneVsOneP2,
            action: BindingAction::Jump,
        };

        config.set_binding(target, KeyCode::F);

        assert_eq!(config.binding(target), KeyCode::F);
    }

    #[test]
    fn storage_strings_round_trip_default_keys() {
        for key in [
            KeyCode::Q,
            KeyCode::D,
            KeyCode::Z,
            ...
            let stored = keycode_to_storage_string(key);
            assert_eq!(keycode_from_storage_string(&stored), Some(key));
        }
    }
}
