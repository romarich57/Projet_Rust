...
const CROSSBAR_MIN_REBOUND_SPEED: f32 = 1.8;
const CROSSBAR_TOP_EPSILON: f32 = 0.35;
const CROSSBAR_STATIC_ESCAPE_VX: f32 = 0.55;
...
mod tests {
    ...
    #[test]
    fn ball_bounces_off_goal_crossbar() {
        let arena = ArenaGeometry::from_screen(1000.0, 600.0);
        let center = vec2(
            arena.left_goal.crossbar_rect.center().x,
            arena.left_goal.crossbar_rect.bottom() + 6.0,
        );
        let (normal, _) =
            circle_rect_collision_at(center, 12.0, arena.left_goal.crossbar_rect).unwrap();
        let reflected = reflect_velocity(vec2(0.0, -4.0), normal, 0.78);

        assert!(reflected.y > 0.0);
    }

    #[test]
    fn stationary_ball_on_crossbar_is_forced_back_upward() {
        let arena = ArenaGeometry::from_screen(1000.0, 600.0);
        let overlap_center = vec2(
            arena.left_goal.crossbar_rect.center().x,
            arena.left_goal.crossbar_rect.y - 11.5,
        );

        let (corrected_center, corrected_velocity) = crossbar_top_bounce_state(
            overlap_center,
            12.0,
            vec2(0.0, 0.0),
        ...
        assert!(
            ...
            "expected upward rebound, got {}",
            corrected_velocity.y
        );
        assert!(corrected_velocity.x.abs() >= CROSSBAR_STATIC_ESCAPE_VX);
        assert!(corrected_center.y + 12.0 <= arena.left_goal.crossbar_rect.y + 0.01);
    }

    #[test]
    fn ball_bounces_off_back_post() {
        let arena = ArenaGeometry::from_screen(1000.0, 600.0);
        let center = vec2(
            arena.left_goal.back_post_rect.right() + 6.0,
            arena.left_goal.back_post_rect.y + 30.0,
        );
        let (normal, _) =
            circle_rect_collision_at(center, 12.0, arena.left_goal.back_post_rect).unwrap();
        let reflected = reflect_velocity(vec2(-5.0, 0.0), normal, 0.78);

        assert!(reflected.x > 0.0);
    }

    #[test]
    fn ball_already_inside_goal_cannot_escape_through_mouth() {
        let arena = ArenaGeometry::from_screen(1000.0, 600.0);
        let retained = retained_ball_state(
            arena.left_goal.mouth_line_x - 8.0,
            12.0,
            4.0,
            arena.left_goal,
        )
        ...
        assert!(retained.0 + 12.0 <= arena.left_goal.mouth_line_x + 0.001);
        assert!(retained.1 <= 0.0);
    }
}
